...
def plot_topics(rosbag_path: Path, topics: list[str], label_map: dict[str, str], outpath: Path) -> None:
    ...
    for topic, msg, t in bag.read_messages(topics=topics):
        # Try to extract a numeric field or a list/vector from the message
        value = getattr(msg, 'data', None)
        if value is not None:
            # If value is a list, tuple, or numpy array, treat as vector
            ...
            # Try common vector fields (e.g., geometry_msgs/Vector3)
    ...
    plt.ylabel("Value")
    plt.legend()
    # plt.title("ROS Topic Data from Bag")
    plt.tight_layout()
    # plt.show()
    plt.savefig(outpath)
    plt.close()
...
def main() -> None:
    ...
    for bag_path in rosbags:
        ...
        outpath = (Path(src.__file__).parent.parent / "images" / bag_path.stem).with_suffix('.png')
